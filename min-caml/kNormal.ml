...
let deInt = function
  | _, Syntax.Int n -> n
  | _ -> failwith "kNormal.deInt: here only integer literal is allowed"

let rec g toplevel addrenv lines env (range, body) = match body with (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.If(e1, e2, e3) -> g toplevel addrenv lines env (range, Syntax.If((range, Syntax.Eq(e1, (range, Syntax.Bool(false)))), e3, e2)) (* 比較のない分岐を変換 (caml2html: knormal_if) *)
  ...
  | Syntax.Array(e1, e2) ->
      ...
      let _ = if toplevel then
        let d = if !H.boundary_check then 1 else 1 in
