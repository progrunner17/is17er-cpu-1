...
let rec g env (range, body) = match body with (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  ...
  | Closure.IfEq(x, y, e1, e2) ->
      ...
      | Type.Bool | Type.Int -> range, Ans(range, IfEq(x, V(y), g env e1, g env e2))
      | Type.Float -> range, Ans(range, IfFEq(x, y, g env e1, g env e2))
      | _ -> failwith "equality supported only for bool, int, and float")
  | Closure.IfLE(x, y, e1, e2) ->
      (match M.find x env with
      | Type.Bool | Type.Int -> range, Ans(range, IfLE(x, V(y), g env e1, g env e2))
      | Type.Float -> range, Ans(range, IfFLE(x, y, g env e1, g env e2))
      | _ -> failwith "inequality supported only for bool, int, and float")
  | Closure.Let((x, t1), e1, e2) ->
      let e1' = g env e1 in
      let e2' = g (M.add x t1 env) e2 in
      concat range e1' (x, t1) e2'
  | Closure.Var(x) ->
      (match M.find x env with
  ...
  | Closure.MakeCls((x, t), { Closure.entry = l; Closure.actual_fv = ys }, e2) -> (* クロージャの生成 (caml2html: virtual_makecls) *)
      ...
      range, Let((x, t), (range, Mr(reg_hp)),
          (range, Let((reg_hp, Type.Int), (range, Add(reg_hp, C(align offset))),
              let z = Id.genid "l" in
              (range, Let((z, Type.Int), (range, SetL(l)),
                  seq(range, (range, Stw(z, x, C(0))),
                      store_fv))))))
  | Closure.AppCls(x, ys) ->
  ...
  | Closure.Tuple(xs) -> (* 組の生成 (caml2html: virtual_tuple) *)
      ...
      range, Let((y, Type.Tuple(List.map (fun x -> M.find x env) xs)), (range, Mr(reg_hp)),
          (range, Let((reg_hp, Type.Int), (range, Add(reg_hp, C(align offset))),
              store)))
      ...
      let (offset, load) =
        expand
          ...
          (fun x offset load ->
            ...
            fletd(range, x, (range, Lfd(y, C(offset))), load))
          (fun x t offset load ->
            if not (S.mem x s) then load else (* [XX] a little ad hoc optimization *)
            range, Let((x, t), (range, Lwz(y, C(offset))), load)) in
      load
  | Closure.Get(x, y) -> (* 配列の読み出し (caml2html: virtual_get) *)
      ...
      | Type.Array(Type.Float) ->
          range, Let((offset, Type.Int), (range, Slw(y, C(3))),
              (range, Ans(range, Lfd(x, V(offset)))))
      | Type.Array(_) ->
          range, Let((offset, Type.Int), (range, Slw(y, C(2))),
...
let h { Closure.range = range; Closure.name = (Id.L(x), t); Closure.args = yts; Closure.formal_fv = zts; Closure.body = e } =
  ...
  let (offset, load) =
    expand
      ...
      (fun z offset load -> fletd(range, z, (range, Lfd(x, C(offset))), load))
      (fun z t offset load -> range, Let((z, t), (range, Lwz(x, C(offset))), load)) in
  match t with
  | Type.Fun(_, t2) ->
