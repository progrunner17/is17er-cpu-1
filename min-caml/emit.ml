...
(* MATSUSHITA: added comment_range function *)
let comment_range = function
  | None -> ""
...
let load_label range r label =
  ...
  Printf.sprintf
    "\tlis\t%s, ha16(%s)%s\n\taddi\t%s, %s, lo16(%s)%s\n"
    r' label (comment_range range) r' r' label (comment_range range)
...
and g' range oc (dest, ((_, body) as exp)) = match (dest, body) with (* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  ...
  | NonTail(_), Save(x, y) when List.mem x allregs && not (S.mem y !stackset) ->
      save y;
      Printf.fprintf oc "\tstw\t%s, %d(%s)%s\n" (reg x) (offset y) (reg reg_sp) (comment_range range)
  ...
  | Tail, CallCls(x, ys, zs) -> (* 末尾呼び出し (caml2html: emit_tailcall) *)
      ...
      Printf.fprintf oc "\tlwz\t%s, 0(%s)%s\n" (reg reg_sw) (reg reg_cl) (comment_range range);
      Printf.fprintf oc "\tmtctr\t%s\n\tbctr%s\n" (reg reg_sw) (comment_range range);
  | Tail, CallDir(Id.L(x), ys, zs) -> (* 末尾呼び出し *)
      g'_args range oc [] ys zs;
      Printf.fprintf oc "\tb\t%s%s\n" x (comment_range range)
  | NonTail(a), CallCls(x, ys, zs) ->
      Printf.fprintf oc "\tmflr\t%s%s\n" (reg reg_tmp) (comment_range range);
      g'_args range oc [(x, reg_cl)] ys zs;
      let ss = stacksize () in
      Printf.fprintf oc "\tstw\t%s, %d(%s)%s\n" (reg reg_tmp) (ss - 4) (reg reg_sp) (comment_range range);
      Printf.fprintf oc "\taddi\t%s, %s, %d%s\n" (reg reg_sp) (reg reg_sp) ss (comment_range range);
      Printf.fprintf oc "\tlwz\t%s, 0(%s)%s\n" (reg reg_tmp) (reg reg_cl) (comment_range range);
      Printf.fprintf oc "\tmtctr\t%s%s\n" (reg reg_tmp) (comment_range range);
      Printf.fprintf oc "\tbctrl%s\n" (comment_range range);
      Printf.fprintf oc "\tsubi\t%s, %s, %d%s\n" (reg reg_sp) (reg reg_sp) ss (comment_range range);
      Printf.fprintf oc "\tlwz\t%s, %d(%s)%s\n" (reg reg_tmp) (ss - 4) (reg reg_sp) (comment_range range);
      if List.mem a allregs && a <> regs.(0) then
        Printf.fprintf oc "\tmr\t%s, %s%s\n" (reg a) (reg regs.(0)) (comment_range range)
      else if List.mem a allfregs && a <> fregs.(0) then
        Printf.fprintf oc "\tfmr\t%s, %s%s\n" (reg a) (reg fregs.(0)) (comment_range range);
...
and g'_tail_if range oc ((range1, _) as e1) ((range2, _) as e2) b bn =
  ...
  Printf.fprintf oc "\t%s\tcr7, %s%s\n" bn b_else (comment_range range);
  Printf.fprintf oc "#\t%s\n" (H.show_range range1);
  let stackset_back = !stackset in
  g oc (Tail, e1);
  Printf.fprintf oc "%s:%s\n" b_else (comment_range range2);
