...
int utoi32(int unsigned_rv){
	int retval;
	if(unsigned_rv<pow(2,31)){
		retval=unsigned_rv;
	}else{
		retval=-(pow(2,32)-unsigned_rv);
	}
//	printf("utoi32\n");
	return retval;
}

int sign32(int n,int sign){
	int sum=0;
	int i,retval;
	for(i=0;i<n;i++)
		sum=sum+pow(2,i);
...
*/
			unsigned_rv=bintonm(instr,12,30)
				+sign32(13,sign)*pow(2,19);
			retval=utoi32(unsigned_rv);
		break;
		case OP_JALR:
		case OP_LOAD:
		case OP_ALUI:
		case OP_LOAD_FP:
			unsigned_rv=bintonm(instr,20,30)
				+sign32(21,sign)*pow(2,11);
			retval=utoi32(unsigned_rv);
		break;
		case OP_BRANCH:
		case OP_STORE:
		case OP_STORE_FP:
 			unsigned_rv=bintonm(instr,25,30)*pow(2,5)
				+bintonm(instr,7,11)
				+sign32(21,sign)*pow(2,11);
			retval=utoi32(unsigned_rv);
/*
