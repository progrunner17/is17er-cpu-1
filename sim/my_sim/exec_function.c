...
word w;
//デバッグ用フラグ
int debug=0;//pc,opcode,funct3(7)を出力。
...
//デバッグ↓
		if(debug){
			if(/*opcode==OP_LOAD_IO&&*/dc==234068)break;
			/*if(opcode==OP_LOAD_IO)*/dc++;
		}
//デバッグ↑
...
//デバッグ↓
		if(debug){
			print_asm(opcode,funct3,funct7,imm,rs1,rs2,rd,shamt,mac,dc);
		}
//デバッグ↑
		switch(opcode){
			case OP_LUI:
