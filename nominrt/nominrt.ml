...
(* sin はなぜか負の値を渡されないが、割と大きい値が来る *)
let sin x =
  let n = int_of_float (x /. pi) in
  (1. -. float_of_int (n mod 2) *. 2.) *. cos (x -. float_of_int n *. pi -. pi /. 2.)
...
let tan x =
  ...
  let t3 = x *. xx /. 3. in
  let t5 = t3 *. xx *. (2. /. 5.) in
  let t7 = t5 *. xx *. (17. /. 42.) in
  let t9 = t7 *. xx *. (62. /. 153.) in
  x +. t3 +. t5 +. t7 +. t9

(* atan には 1 から 3 くらいまでの値が来るので 2 の周りで Taylor 展開した*)
let atan x =
  let t0 = 1.10714872 in
  let t1 = (x -. 2.) /. 5. in
  let t2 = t1 *. t1 *. 2. in
  let t3 = t2 *. t1 *. (11. /. 6.) in
  let t4 = t3 *. t1 *. (18. /. 11.) in
  let t5 = t4 *. t1 *. (41. /. 30.) in
  t0 +. t1 -. t2 +. t3 -. t4 +. t5
